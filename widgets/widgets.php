...
function origin_widgets_init(){
	foreach(glob(plugin_dir_path(__FILE__).'/widgets/*/*.php') as $file) {
		...
		$class = 'DistinctiveThemes_Panels_Widget_'.$class;
...
function origin_widgets_enqueue($prefix){
	if($prefix == 'widgets.php') wp_enqueue_script('origin-widgets-admin-script', plugin_dir_url(DISTINCTIVETHEMES_PANELS_BASE_FILE).'/widgets/js/admin.js', array('jquery'), DISTINCTIVETHEMES_PANELS_VERSION);
}
...
function origin_widgets_generate_css($class, $style, $preset, $version = null){
	$widget = new $class();
	if( !is_subclass_of($widget, 'DistinctiveThemes_Panels_Widget') ) return '';
	if(empty($version)) $version = DISTINCTIVETHEMES_PANELS_VERSION;

	$id = str_replace('_', '', strtolower(str_replace('DistinctiveThemes_Panels_Widget_', '', $class)));
	$key = strtolower($id.'-'.$style.'-'. $preset.'-'.str_replace('.', '', $version));

	$css = get_site_transient('origin_wcss:'.$key);
	if($css === false || ( defined('DISTINCTIVETHEMES_PANELS_NOCACHE') && DISTINCTIVETHEMES_PANELS_NOCACHE ) ) {
...
/**
 * Class DistinctiveThemes_Panels_Widget
 */
abstract class DistinctiveThemes_Panels_Widget extends WP_Widget{
	public $form_args;
	protected $demo;
	protected $origin_id;
	public $sub_widgets;
	...
	function __construct($name, $widget_options = array(), $control_options = array(), $form = array(), $demo = array()){
		$id_base = str_replace('DistinctiveThemes_Panels_Widget_', '', get_class($this));
	...
	function update($new, $old) {
		...
		if(!distinctivethemes_panels_is_preview()){
	...
	public function form($instance){
		...
		do_action('distinctivethemes_panels_widget_before_styles', $this, $instance);
		...
		if( !empty( $styles ) ) {
			?>
			<p>
				<label for="<?php echo $this->get_field_id('origin_style') ?>"><?php _e('Style', 'distinctivethemes-panels') ?></label>
		...
		do_action('distinctivethemes_panels_widget_before_substyles', $this, $instance);
		...
		foreach($this->sub_widgets as $id => $sub) {
			...
			?>
			<p>
				<label for="<?php echo $this->get_field_id('origin_style_'.$id) ?>"><?php printf(__('%s Style', 'distinctivethemes-panels'), $sub[0]) ?></label>
		...
		do_action('distinctivethemes_panels_widget_after_styles', $this, $instance);
	}
	...
	function widget($args, $instance){
		...
		$widget_classes = apply_filters('distinctivethemes_widgets_classes', array(
			'origin-widget',
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	// Extra functions specific to a DistinctiveThemes: widget.
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	static function get_widget_folders(){
		...
		if(empty($folders)) {
			$folders = array(
				...
				plugin_dir_path(DISTINCTIVETHEMES_PANELS_BASE_FILE).'widgets/widgets' => plugin_dir_url(DISTINCTIVETHEMES_PANELS_BASE_FILE).'widgets/widgets',
			);
			$folders = apply_filters('distinctivethemes_widget_folders', $folders);
		}
	...
	static function get_image_folders(){
		...
		if(empty($folders)) {
			$folders = array(
				...
				plugin_dir_path(DISTINCTIVETHEMES_PANELS_BASE_FILE).'widgets/img' => plugin_dir_url(DISTINCTIVETHEMES_PANELS_BASE_FILE).'widgets/img',
			);
			$folders = apply_filters('distinctivethemes_widget_image_folders', $folders);
		}
	...
	function add_post_query_fields(){
		...
		$this->form_args['query_post_type'] = array(
			'type' => 'select',
			'options' => $post_types,
			'label' => __('Post Type', 'distinctivethemes-panels')
		);
		...
		$this->form_args['query_posts_per_page'] = array(
			'type' => 'number',
			'default' => 10,
			'label' => __('Posts Per Page', 'distinctivethemes-panels'),
		);

		$this->form_args['query_orderby'] = array(
			'type' => 'select',
			'label' => __('Order By', 'distinctivethemes-panels'),
			'options' => array(
				'none'  => __('None', 'distinctivethemes-panels'),
				'ID'  => __('Post ID', 'distinctivethemes-panels'),
				'author'  => __('Author', 'distinctivethemes-panels'),
				'name'  => __('Name', 'distinctivethemes-panels'),
				'name'  => __('Name', 'distinctivethemes-panels'),
				'date'  => __('Date', 'distinctivethemes-panels'),
				'modified'  => __('Modified', 'distinctivethemes-panels'),
				'parent'  => __('Parent', 'distinctivethemes-panels'),
				'rand'  => __('Random', 'distinctivethemes-panels'),
				'comment_count'  => __('Comment Count', 'distinctivethemes-panels'),
				'menu_order'  => __('Menu Order', 'distinctivethemes-panels'),
			)
		);

		$this->form_args['query_order'] = array(
			'type' => 'select',
			'label' => __('Order', 'distinctivethemes-panels'),
			'options' => array(
				'ASC'  => __('Ascending', 'distinctivethemes-panels'),
				'DESC'  => __('Descending', 'distinctivethemes-panels'),
			)
		);

		$this->form_args['query_sticky'] = array(
			'type' => 'select',
			'label' => __('Sticky Posts', 'distinctivethemes-panels'),
			'options' => array(
				''  => __('Default', 'distinctivethemes-panels'),
				'ignore'  => __('Ignore Sticky', 'distinctivethemes-panels'),
				'exclude'  => __('Exclude Sticky', 'distinctivethemes-panels'),
				'only'  => __('Only Sticky', 'distinctivethemes-panels'),
			)
		);

		$this->form_args['query_additional'] = array(
			'type' => 'text',
			'label' => __('Additional Arguments', 'distinctivethemes-panels'),
			'description' => preg_replace(
				'/1\{ *(.*?) *\}/',
				'<a href="http://codex.wordpress.org/Function_Reference/query_posts">$1</a>',
				__('Additional query arguments. See 1{query_posts}.', 'distinctivethemes-panels')
			)
		);
	}
 ...
 * A gallery widget
 *
 * Class DistinctiveThemes_Panels_Widgets_Gallery
 */
class DistinctiveThemes_Panels_Widgets_Gallery extends WP_Widget {
	function __construct() {
		parent::__construct(
			'distinctivethemes-panels-gallery',
			__( 'Gallery (PB)', 'distinctivethemes-panels' ),
			array(
				'description' => __( 'Displays a gallery.', 'distinctivethemes-panels' ),
			)
		);
	}
	...
	function form( $instance ) {
		...
		$types = apply_filters('distinctivethemes_panels_gallery_types', array());

		$instance = wp_parse_args($instance, array(
			'ids' => '',
			'size' => apply_filters('distinctivethemes_panels_gallery_default_size', ''),
			'type' => apply_filters('distinctivethemes_panels_gallery_default_type', ''),
			'columns' => 3,
			'link' => '',
		...
		?>
		<p>
			<label for="<?php echo $this->get_field_id( 'ids' ) ?>"><?php _e( 'Gallery Images', 'distinctivethemes-panels' ) ?></label>
			<a href="#" onclick="return false;" class="so-gallery-widget-select-attachments hidden"><?php _e('edit gallery', 'distinctivethemes-panels') ?></a>
			<input type="text" class="widefat" value="<?php echo esc_attr($instance['ids']) ?>" name="<?php echo $this->get_field_name('ids') ?>" />
		</p>
		<p class="description">
			<?php _e("Comma separated attachment IDs. Defaults to all current page's attachments.", 'distinctivethemes-panels') ?>
		</p>

		<p>
			<label for="<?php echo $this->get_field_id( 'size' ) ?>"><?php _e( 'Image Size', 'distinctivethemes-panels' ) ?></label>
			<select name="<?php echo $this->get_field_name( 'size' ) ?>" id="<?php echo $this->get_field_id( 'size' ) ?>">
				<option value="" <?php selected(empty($instance['size'])) ?>><?php esc_html_e('Default', 'distinctivethemes-panels') ?></option>
				<option value="large" <?php selected('large', $instance['size']) ?>><?php esc_html_e( 'Large', 'distinctivethemes-panels' ) ?></option>
				<option value="medium" <?php selected('medium', $instance['size']) ?>><?php esc_html_e( 'Medium', 'distinctivethemes-panels' ) ?></option>
				<option value="thumbnail" <?php selected('thumbnail', $instance['size']) ?>><?php esc_html_e( 'Thumbnail', 'distinctivethemes-panels' ) ?></option>
				<option value="full" <?php selected('full', $instance['size']) ?>><?php esc_html_e( 'Full', 'distinctivethemes-panels' ) ?></option>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'type' ) ?>"><?php _e( 'Gallery Type', 'distinctivethemes-panels' ) ?></label>
			<input type="text" class="regular" value="<?php echo esc_attr($instance['type']) ?>" name="<?php echo $this->get_field_name('type') ?>" />
		</p>

		<p>
			<label for="<?php echo $this->get_field_id( 'columns' ) ?>"><?php _e( 'Columns', 'distinctivethemes-panels' ) ?></label>
			<input type="text" class="regular" value="<?php echo esc_attr($instance['columns']) ?>" name="<?php echo $this->get_field_name('columns') ?>" />
		</p>

		<p>
			<label for="<?php echo $this->get_field_id( 'link' ) ?>"><?php _e( 'Link To', 'distinctivethemes-panels' ) ?></label>
			<select name="<?php echo $this->get_field_name( 'link' ) ?>" id="<?php echo $this->get_field_id( 'link' ) ?>">
				<option value="" <?php selected('', $instance['link']) ?>><?php esc_html_e('Attachment Page', 'distinctivethemes-panels') ?></option>
				<option value="file" <?php selected('file', $instance['link']) ?>><?php esc_html_e('File', 'distinctivethemes-panels') ?></option>
				<option value="none" <?php selected('none', $instance['link']) ?>><?php esc_html_e('None', 'distinctivethemes-panels') ?></option>
			</select>
		</p>
 ...
 * An image widget
 *
 * Class DistinctiveThemes_Panels_Widgets_Image
 */
class DistinctiveThemes_Panels_Widgets_Image extends WP_Widget {
	function __construct() {
		parent::__construct(
			'distinctivethemes-panels-image',
			__( 'Image (PB)', 'distinctivethemes-panels' ),
			array(
				'description' => __( 'Displays a simple image.', 'distinctivethemes-panels' ),
			)
		);
	}
	...
	function form( $instance ) {
		...
		?>
		<p>
			<label for="<?php echo $this->get_field_id( 'src' ) ?>"><?php _e( 'Image URL', 'distinctivethemes-panels' ) ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'href' ) ?>"><?php _e( 'Destination URL', 'distinctivethemes-panels' ) ?></label>
 ...
 */
class DistinctiveThemes_Panels_Widgets_EmbeddedVideo extends WP_Widget {
	function __construct() {
		parent::__construct(
			'distinctivethemes-panels-embedded-video',
			__( 'Embedded Video (PB)', 'distinctivethemes-panels' ),
			array(
				'description' => __( 'Embeds a video.', 'distinctivethemes-panels' ),
			)
		);
	}
	...
	function widget( $args, $instance ) {
		...
		if(!wp_script_is('fitvids'))
			wp_enqueue_script('fitvids', plugin_dir_url(DISTINCTIVETHEMES_PANELS_BASE_FILE).'widgets/js/jquery.fitvids.js', array('jquery'), DISTINCTIVETHEMES_PANELS_VERSION);

		if(!wp_script_is('distinctivethemes-panels-embedded-video'))
			wp_enqueue_script('distinctivethemes-panels-embedded-video', plugin_dir_url(DISTINCTIVETHEMES_PANELS_BASE_FILE).'widgets/js/embedded-video.js', array('jquery', 'fitvids'), DISTINCTIVETHEMES_PANELS_VERSION);

		echo $args['before_widget'];
		?><div class="distinctivethemes-fitvids"><?php echo $embed->run_shortcode( '[embed]' . $instance['video'] . '[/embed]' ) ?></div><?php
		echo $args['after_widget'];
	}
	...
	function form( $instance ) {
		...
		?>
		<p>
			<label for="<?php echo $this->get_field_id( 'video' ) ?>"><?php _e( 'Video', 'distinctivethemes-panels' ) ?></label>
...
class DistinctiveThemes_Panels_Widgets_Video extends WP_Widget {
	function __construct() {
		parent::__construct(
			'distinctivethemes-panels-video',
			__( 'Self Hosted Video (PB)', 'distinctivethemes-panels' ),
			array(
				'description' => __( 'A self hosted video player.', 'distinctivethemes-panels' ),
			)
		);
	}
	...
	function form( $instance ) {
		...
		?>
		<p>
			<label for="<?php echo $this->get_field_id('url') ?>"><?php _e('Video URL', 'distinctivethemes-panels') ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('poster') ?>"><?php _e('Poster URL', 'distinctivethemes-panels') ?></label>
			...
			<small class="description"><?php _e('An image that displays before the video starts playing.', 'distinctivethemes-panels') ?></small>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('autoplay') ?>">
				...
				<?php _e('Auto Play Video', 'distinctivethemes-panels') ?>
			</label>
		</p>
	<?php
	}
 ...
 * @return string
 */
function distinctivethemes_panels_video_shortcode($atts){
	/**
	 * @var string $url
	 * @var string $poster
	 * @var string $skin
	...
	$instance = shortcode_atts( array(
		'url' => '',
		'src' => '',
		'poster' => plugin_dir_url(DISTINCTIVETHEMES_PANELS_BASE_FILE).'video/poster.jpg',
		'skin' => 'distinctivethemes',
		'ratio' => 1.777,
		'autoplay' => 0,
	), $atts );
	...
	ob_start();
	the_widget('DistinctiveThemes_Panels_Widgets_Video', $instance);
	return ob_get_clean();

}
add_shortcode('self_video', 'distinctivethemes_panels_video_shortcode');

/**
 * Register the widgets.
 */
function distinctivethemes_panels_widgets_init(){
	register_widget('DistinctiveThemes_Panels_Widgets_Gallery');
	register_widget('DistinctiveThemes_Panels_Widgets_Image');
	register_widget('DistinctiveThemes_Panels_Widgets_EmbeddedVideo');
	register_widget('DistinctiveThemes_Panels_Widgets_Video');
}
add_action('widgets_init', 'distinctivethemes_panels_widgets_init');

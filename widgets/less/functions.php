...
function origin_widgets_less_lum_change($args, $type = 'darken'){
	if( !class_exists('DistinctiveThemes_Color_Object') ) include plugin_dir_path(__FILE__).'../lib/color.php';
	...
	$color = new DistinctiveThemes_Color_Object($a1_value);
...
function origin_widgets_less_texture($texture){
	...
	foreach($texture[2] as $arg) {
		if($arg[0] == 'keyword') {
			$t = $arg[1];
			if($t == 'none') continue;
			foreach(DistinctiveThemes_Panels_Widget::get_image_folders() as $folder => $folder_url) {

...
class DistinctiveThemes_Panels_Sidebars_Emulator {
	 ...
	 *
	 * @return DistinctiveThemes_Panels_Widgets
	 */
	static function single(){
		static $single = false;
		if( empty($single) ) $single = new DistinctiveThemes_Panels_Sidebars_Emulator();
	...
	/**
	 * Add a sidebar for DistinctiveThemes: Panels widgets so they are correctly detected by is_active_widget
	 *
	...
	function add_widgets_to_sidebars( $sidebars_widgets ) {
		...
		foreach ( array_keys( $this->all_posts_widgets ) as $post_id ) {
			...
			foreach ( $post_widgets as $widget_instance ) {
				...
				$distinctivethemes_panels_widget_ids[] = $widget_instance['id'];
			}
			if( ! empty( $distinctivethemes_panels_widget_ids) ) $sidebars_widgets['sidebar-distinctivethemes_panels-post-' . $post_id] = $distinctivethemes_panels_widget_ids;
		}
...
DistinctiveThemes_Panels_Sidebars_Emulator::single();

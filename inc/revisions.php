 ...
 * @param $post
 */
function distinctivethemes_panels_revisions_save_post( $post_id, $post ) {
...
add_action( 'save_post', 'distinctivethemes_panels_revisions_save_post', 11, 2 );
 ...
 * @param $revision_id
 */
function distinctivethemes_panels_revisions_restore( $post_id, $revision_id ) {
...
add_action( 'wp_restore_post_revision', 'distinctivethemes_panels_revisions_restore', 10, 2 );
 ...
 * @return mixed
 */
function distinctivethemes_panels_revisions_fields( $fields ) {
	...
	$fields['panels_data_field'] = __('Page Builder Content', 'distinctivethemes-panels');
	return $fields;

}
add_filter( '_wp_post_revision_fields', 'distinctivethemes_panels_revisions_fields' );
 ...
 * @return string
 */
function distinctivethemes_panels_revisions_field( $value, $field, $revision ) {
	...
	return distinctivethemes_panels_render($parent_id, false, $panels_data);
}
add_filter( '_wp_post_revision_field_panels_data_field', 'distinctivethemes_panels_revisions_field', 10, 3 );

...
define('DISTINCTIVETHEMES_PANELS_LAYOUT_URL', 'http://layouts.distinctivethemes.com/');

/**
 * Get builder content based on the submitted panels_data.
 */
function distinctivethemes_panels_ajax_builder_content(){
	...
	echo distinctivethemes_panels_render( intval($_POST['post_id']), false, $panels_data );

	wp_die();
}
add_action('wp_ajax_so_panels_builder_content', 'distinctivethemes_panels_ajax_builder_content');

/**
 * Display a widget form with the provided data
 */
function distinctivethemes_panels_ajax_widget_form(){
	...
	$form = distinctivethemes_panels_render_form( $widget, $instance, $_REQUEST['raw'] == 'true' );
	$form = apply_filters('distinctivethemes_panels_ajax_widget_form', $form, $widget, $instance);

	echo $form;
	wp_die();
}
add_action('wp_ajax_so_panels_widget_form', 'distinctivethemes_panels_ajax_widget_form');


function distinctivethemes_panels_ajax_get_prebuilt_layouts(){
	if( empty( $_REQUEST['_panelsnonce'] ) || !wp_verify_nonce($_REQUEST['_panelsnonce'], 'panels_action') ) wp_die();
	...
	if( $type == 'prebuilt' ) {
		$return['title'] = __( 'Theme Defined Layouts', 'distinctivethemes-panels' );
	...
	elseif( $type == 'directory' ) {
		$return['title'] = __( 'Layouts Directory', 'distinctivethemes-panels' );
		...
		$url = add_query_arg( $query, DISTINCTIVETHEMES_PANELS_LAYOUT_URL . '/wp-admin/admin-ajax.php?action=query_layouts');
	...
	elseif ( strpos( $type, 'clone_' ) !== false ) {
		...
		$return['title'] = sprintf( __( 'Clone %s', 'distinctivethemes-panels' ), esc_html( ucfirst( $post_type ) ) );
	...
	if( !empty($search) ) {
		$return['title'] .= __(' - Results For:', 'distinctivethemes-panels') . ' <em>' . esc_html( $search ) . '</em>';
	}
...
add_action('wp_ajax_so_panels_layouts_query', 'distinctivethemes_panels_ajax_get_prebuilt_layouts');

/**
 * Ajax handler to get an individual prebuilt layout
 */
function distinctivethemes_panels_ajax_get_prebuilt_layout(){
	...
	if( $_REQUEST['type'] == 'prebuilt' ) {
		...
		$layout = apply_filters('distinctivethemes_panels_prebuilt_layout', $layout);
		$layout = apply_filters('distinctivethemes_panels_data', $layout);
	...
	if( $_REQUEST['type'] == 'directory' ) {
		$response = wp_remote_get(
			DISTINCTIVETHEMES_PANELS_LAYOUT_URL . '/layout/' . urlencode($_REQUEST['lid']) . '/?action=download'
		);
	...
	elseif( current_user_can('edit_post', $_REQUEST['lid']) ) {
		...
		$panels_data = apply_filters('distinctivethemes_panels_data', $panels_data);
		echo json_encode( $panels_data );
		wp_die();
	}
}
add_action('wp_ajax_so_panels_get_layout', 'distinctivethemes_panels_ajax_get_prebuilt_layout');

/**
 * Ajax handler to import a layout
 */
function distinctivethemes_panels_ajax_import_layout(){
	if( empty( $_REQUEST['_panelsnonce'] ) || !wp_verify_nonce($_REQUEST['_panelsnonce'], 'panels_action') ) wp_die();
...
add_action('wp_ajax_so_panels_import_layout', 'distinctivethemes_panels_ajax_import_layout');

/**
 * Ajax handler to export a layout
 */
function distinctivethemes_panels_ajax_export_layout(){
	if( empty( $_REQUEST['_panelsnonce'] ) || !wp_verify_nonce($_REQUEST['_panelsnonce'], 'panels_action') ) wp_die();
...
add_action('wp_ajax_so_panels_export_layout', 'distinctivethemes_panels_ajax_export_layout');

/**
 * We want users to be informed of what the layout directory is, so they need to enable it.
 */
function distinctivethemes_panels_ajax_directory_enable(){
	if( empty( $_REQUEST['_panelsnonce'] ) || !wp_verify_nonce($_REQUEST['_panelsnonce'], 'panels_action') ) wp_die();
...
add_action('wp_ajax_so_panels_directory_enable', 'distinctivethemes_panels_ajax_directory_enable');

/**
 * Preview in the live editor when there is no public view of the item
 */
function distinctivethemes_panels_live_editor_preview(){
...
add_action('wp_ajax_so_panels_live_editor_preview', 'distinctivethemes_panels_live_editor_preview');

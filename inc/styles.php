 ...
 */
function distinctivethemes_panels_ajax_action_style_form(){
	$type = $_REQUEST['type'];
	...
	switch($type) {
		case 'row':
			distinctivethemes_panels_render_styles_fields('row', '<h3>' . __('Row Styles', 'distinctivethemes-panels') . '</h3>', '', $current, $post_id, $args);
			break;

		case 'widget':
			distinctivethemes_panels_render_styles_fields('widget', '<h3>' . __('Widget Styles', 'distinctivethemes-panels') . '</h3>', '', $current, $post_id, $args);
	}

	wp_die();
}
add_action('wp_ajax_so_panels_style_form', 'distinctivethemes_panels_ajax_action_style_form');
 ...
 * @return bool
 */
function distinctivethemes_panels_render_styles_fields( $section, $before = '', $after = '', $current = array(), $post_id = 0, $args = array() ){
	$fields = apply_filters('distinctivethemes_panels_' . $section . '_style_fields', array(), $post_id, $args );
	if( empty($fields) ) return false;

	$groups = array(
		'attributes' => array(
			'name' => __('Attributes', 'distinctivethemes-panels'),
			'priority' => 5
		),
		'layout' => array(
			'name' => __('Layout', 'distinctivethemes-panels'),
			'priority' => 10
		),
		'design' => array(
			'name' => __('Design', 'distinctivethemes-panels'),
			'priority' => 15
		),
	);
	...
	foreach($fields as $field_id => $field) {
		if( empty($field['group']) || $field['group'] == 'theme' ) {
			if( empty($groups['theme']) ) {
				$groups['theme'] = array(
					'name' => __('Theme', 'distinctivethemes-panels'),
					'priority' => 10
				);
			}
	...
	$groups = apply_filters('distinctivethemes_panels_' . $section . '_style_groups', $groups, $post_id, $args );

	// Sort the style fields and groups by priority
	uasort( $fields, 'distinctivethemes_panels_styles_sort_fields' );
	uasort( $groups, 'distinctivethemes_panels_styles_sort_fields' );
					...
					if($field['group'] == $group_id){
						...
						<div class="style-field-wrapper">
							...
							<div class="style-field style-field-<?php echo sanitize_html_class( $field['type'] ) ?>">
								<?php distinctivethemes_panels_render_style_field( $field, isset( $current[$field_id] ) ? $current[$field_id] : $default, $field_id ) ?>
							</div>
						</div>
						<?php
 ...
 * @return array
 */
function distinctivethemes_panels_style_get_measurements_list() {
	$measurements = array(
		'px',
		'%',
		'in',
	...
	return apply_filters('distinctivethemes_panels_style_get_measurements_list', $measurements);
}
 ...
 * @param $current
 */
function distinctivethemes_panels_render_style_field( $field, $current, $field_id ){
	...
	switch($field['type']) {
		case 'measurement' :
			...
			<input type="text" />
			<select>
				<?php foreach ( distinctivethemes_panels_style_get_measurements_list() as $measurement ):?>
		...
		case 'image' :
			...
			<div class="so-image-selector">
				...
				<div class="select-image">
					<?php _e('Select Image', 'distinctivethemes-panels') ?>
				</div>
				<input type="hidden" name="<?php echo esc_attr($field_name) ?>" value="<?php echo intval($current) ?>" />
			</div>
			<a href="#" class="remove-image"><?php _e('Remove', 'distinctivethemes-panels') ?></a>
			<?php
			break;
		...
		case 'checkbox' :
			...
			<label class="so-checkbox-label">
				...
				<?php echo esc_html( isset($field['label']) ? $field['label'] : __('Enabled', 'distinctivethemes-panels') ) ?>
			</label>
			<?php
			break;
 ...
 * @return int
 */
function distinctivethemes_panels_styles_sort_fields($a, $b){
 ...
 * @return mixed
 */
function distinctivethemes_panels_styles_sanitize_all($panels_data){
	if( !empty($panels_data['widgets']) ) {
		...
		for ( $i = 0; $i < count( $panels_data['widgets'] ); $i ++ ) {
			...
			$panels_data['widgets'][ $i ]['panels_info']['style'] = distinctivethemes_panels_sanitize_style_fields( 'widget', $panels_data['widgets'][ $i ]['panels_info']['style'] );
		}
	}
	...
	if( !empty($panels_data['grids']) ) {
		...
		for ( $i = 0; $i < count( $panels_data['grids'] ); $i ++ ) {
			...
			$panels_data['grids'][ $i ]['style'] = distinctivethemes_panels_sanitize_style_fields( 'row', $panels_data['grids'][ $i ]['style'] );
		}
	}
	...
	if( !empty($panels_data['grid_cells']) ) {
		...
		for ( $i = 0; $i < count( $panels_data['grid_cells'] ); $i ++ ) {
			...
			$panels_data['grid_cells'][ $i ]['style'] = distinctivethemes_panels_sanitize_style_fields( 'cell', $panels_data['grid_cells'][ $i ]['style'] );
		}
	}
 ...
 */
function distinctivethemes_panels_sanitize_style_fields( $section, $styles ){
	...
	if( empty($fields_cache[$section]) ) {
		...
		$fields_cache[$section] = apply_filters('distinctivethemes_panels_' . $section . '_style_fields', array(), false, false );
	}
	...
	foreach($fields as $k => $field) {
		...
		switch($field['type']) {
			...
			case 'url' :
				...
				break;
			case 'measurement' :
				$measurements = array_map('preg_quote', distinctivethemes_panels_style_get_measurements_list() );
 ...
 * @return mixed
 */
function distinctivethemes_panels_style_update_data($panels_data){
...
add_filter('distinctivethemes_panels_data', 'distinctivethemes_panels_style_update_data');
add_filter('distinctivethemes_panels_prebuilt_layout', 'distinctivethemes_panels_style_update_data');

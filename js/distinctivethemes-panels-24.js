...
module.exports = panels.view.dialog.extend( {
	...
	render: function () {
		...
		this.renderDialog( this.parseDialogContent( $( '#distinctivethemes-panels-dialog-builder' ).html(), {} ) );
		this.$( '.so-content .distinctivethemes-panels-builder' ).append( this.builder.$el );
	},
...
module.exports = panels.view.dialog.extend( {
	...
	historyEntryTemplate: _.template( $( '#distinctivethemes-panels-dialog-history-entry' ).html().panelsProcessTemplate() ),
	...
	render: function () {
		...
		// Render the dialog and attach it to the builder interface
		this.renderDialog( this.parseDialogContent( $( '#distinctivethemes-panels-dialog-history' ).html(), {} ) );

		this.$( 'iframe.distinctivethemes-panels-history-iframe' ).load( function () {
			var $$ = $( this );
			$$.show();
	...
	previewEntry: function ( entry ) {
		var iframe = this.$( 'iframe.distinctivethemes-panels-history-iframe' );
		iframe.hide();
...
module.exports = panels.view.dialog.extend( {
	...
	directoryTemplate: _.template( $( '#distinctivethemes-panels-directory-items' ).html().panelsProcessTemplate() ),
	 ...
	 */
	render: function () {
		this.renderDialog( this.parseDialogContent( $( '#distinctivethemes-panels-dialog-prebuilt' ).html(), {} ) );
	...
	displayImportExport: function () {
		...
		c.html( $( '#distinctivethemes-panels-dialog-prebuilt-importexport' ).html() );
	...
	displayLayoutDirectory: function ( search, page, type ) {
		...
		if ( type === 'directory' && ! panelsOptions.directory_enabled ) {
			...
			c.removeClass( 'so-panels-loading' ).html( $( '#distinctivethemes-panels-directory-enable' ).html() );
...
module.exports = panels.view.dialog.extend( {
	...
	cellPreviewTemplate: _.template( $( '#distinctivethemes-panels-dialog-row-cell-preview' ).html().panelsProcessTemplate() ),
	...
	render: function ( dialogType ) {
		this.renderDialog( this.parseDialogContent( $( '#distinctivethemes-panels-dialog-row' ).html(), {dialogType: this.dialogType} ) );
...
module.exports = panels.view.dialog.extend( {
	...
	builder: null,
	sidebarWidgetTemplate: _.template( $( '#distinctivethemes-panels-dialog-widget-sidebar-widget' ).html().panelsProcessTemplate() ),
	...
	render: function () {
		// Render the dialog and attach it to the builder interface
		this.renderDialog( this.parseDialogContent( $( '#distinctivethemes-panels-dialog-widget' ).html(), {} ) );
		this.loadForm();
...
module.exports = panels.view.dialog.extend( {
	...
	builder: null,
	widgetTemplate: _.template( $( '#distinctivethemes-panels-dialog-widgets-widget' ).html().panelsProcessTemplate() ),
	filter: {},
	...
	render: function () {
		// Render the dialog and attach it to the builder interface
		this.renderDialog( this.parseDialogContent( $( '#distinctivethemes-panels-dialog-widgets' ).html(), {} ) );
...
module.exports = function ( config ) {
	...
	return this.each( function () {
		...
		if ( isWidget || $$.data('mode') === 'dialog' ) {
			...
			$$.find( '.distinctivethemes-panels-display-builder' ).click( function ( e ) {
				e.preventDefault();
				builderView.dialog.openDialog();
			} );
		} else {
			// Remove the dialog opener button, this is already being displayed in a page builder dialog.
			$$.find( '.distinctivethemes-panels-display-builder' ).parent().remove();
		}
...
/**
 * Everything we need for Distinctive Themes Widget Page Builder - based on DistinctiveThemess.
 *
 * @copyright Greg Priday 2013 - 2016 - <https://distinctivethemes.com/>
...
window.distinctivethemesPanels = panels;
...
jQuery( function ( $ ) {
	...
	if ( $( '#distinctivethemes-panels-metabox' ).length && $( 'form#post' ).length ) {
		// This is usually the case when we're in the post edit interface
		container = $( '#distinctivethemes-panels-metabox' );
		field = $( '#distinctivethemes-panels-metabox .distinctivethemes-panels-data-field' );
		form = $( 'form#post' );
		...
		builderConfig = {
			...
			postId: $( '#post_ID' ).val(),
			editorId: '#content',
			builderType: $( '#distinctivethemes-panels-metabox' ).data( 'builder-type' ),
			builderSupports: $( '#distinctivethemes-panels-metabox' ).data( 'builder-supports' ),
			loadLiveEditor: $( '#distinctivethemes-panels-metabox' ).data('live-editor') == 1,
			liveEditorPreview: container.data('preview-url')
		};
	}
	else if ( $( '.distinctivethemes-panels-builder-form' ).length ) {
		// We're dealing with another interface like the custom home page interface
		var $$ = $( '.distinctivethemes-panels-builder-form' );

		container = $$.find( '.distinctivethemes-panels-builder-container' );
	...
	if ( ! _.isUndefined( container ) ) {
		...
		var panels = window.distinctivethemesPanels;
	...
	$( document ).on( 'widget-added', function ( e, widget ) {
		$( widget ).find( '.distinctivethemes-page-builder-widget' ).soPanelsSetupBuilderWidget();
	} );

	// Setup existing widgets on the page (for the widgets interface)
	if ( ! $( 'body' ).hasClass( 'wp-customizer' ) ) {
		$( function () {
			$( '.distinctivethemes-page-builder-widget' ).soPanelsSetupBuilderWidget();
		} );
	}
} );
...
module.exports = Backbone.Model.extend( {
	...
	clone: function ( cell, options ) {
		...
		if ( this.get( 'class' ) === "DistinctiveThemes_Panels_Widgets_Layout" ) {
...
module.exports = Backbone.View.extend( {
	wrapperTemplate: _.template( $( '#distinctivethemes-panels-context-menu' ).html().panelsProcessTemplate() ),
	sectionTemplate: _.template( $( '#distinctivethemes-panels-context-menu-section' ).html().panelsProcessTemplate() ),
...
module.exports = Backbone.View.extend( {
	...
	template: _.template( $( '#distinctivethemes-panels-builder' ).html().panelsProcessTemplate() ),
	dialogs: {},
	rowsSortable: null,
	dataField: false,
	currentData: '',
	...
	render: function () {
		...
		this.$el
			.attr( 'id', 'distinctivethemes-panels-builder-' + this.cid )
	...
	handleDisplayBuilder: function () {
		...
		if (
			...
			if ( ! _.isUndefined( panelsOptions.widgets.DistinctiveThemes_Widget_Editor_Widget ) ) {
				widgetClass = 'DistinctiveThemes_Widget_Editor_Widget';
			}
	...
	activateContextMenu: function ( e, menu ) {
		...
		var topmostBuilder = $( '.distinctivethemes-panels-builder:visible' )
			.sort( function ( a, b ) {
...
module.exports = Backbone.View.extend( {
	template: _.template( $( '#distinctivethemes-panels-builder-cell' ).html().panelsProcessTemplate() ),
	events: {
...
module.exports = Backbone.View.extend( {
	dialogTemplate: _.template( $( '#distinctivethemes-panels-dialog' ).html().panelsProcessTemplate() ),
	dialogTabTemplate: _.template( $( '#distinctivethemes-panels-dialog-tab' ).html().panelsProcessTemplate() ),
...
module.exports = Backbone.View.extend( {
	template: _.template( $( '#distinctivethemes-panels-live-editor' ).html().panelsProcessTemplate() ),
	...
	postToIframe: function( data, url, target ){
		...
		var iframeId = 'distinctivethemes-panels-live-preview-' + this.previewFrameId;
...
module.exports = Backbone.View.extend( {
	template: _.template( $( '#distinctivethemes-panels-builder-row' ).html().panelsProcessTemplate() ),
...
module.exports = Backbone.View.extend( {
	template: _.template( $( '#distinctivethemes-panels-builder-widget' ).html().panelsProcessTemplate() ),

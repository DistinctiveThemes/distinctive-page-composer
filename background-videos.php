...
class Background_Videos_For_distinctivethemes {
	...
	function __construct() {
		...
		add_filter( 'distinctivethemes_panels_row_style_groups', array( $this, 'custom_row_style_group' ), 10, 3 );

		// Add fields to row
		add_filter( 'distinctivethemes_panels_row_style_fields', array( $this, 'custom_row_style_fields' ) );

		// Adjust row output
		add_filter( 'distinctivethemes_panels_after_row', array( $this, 'video_after_row' ), 10, 3 );

		// Add a prebuilt layout
		add_filter( 'distinctivethemes_panels_prebuilt_layouts', array( $this, 'bgvideo_prebuilt_layouts' ) );
	...
	public function custom_row_style_group( $groups, $post_id, $args ) {
		...
		$groups['bgvideo'] = array(
			'name' => __('Background Video', 'distinctivethemes-panels'),
			'priority' => 25
		);
	...
	public function custom_row_style_fields( $fields ) {
		...
		$fields['webm_url'] = array(
			'name'        => __('WebM URL', 'distinctivethemes-panels'),
			'type'        => 'url',
			'group'       => $group,
			'description' => __('Recommended file type.', 'distinctivethemes-panels'),
			'priority'    => $order ++,
			'default'     => ''
		);
		...
		$fields['ogg_url'] = array(
			'name'        => __('Ogg URL', 'distinctivethemes-panels'),
			'type'        => 'url',
			'group'       => $group,
			'description' => __('Optional file type.', 'distinctivethemes-panels'),
			'priority'    => $order ++,
			'default'     => ''
		);
/*
		...
		$fields['poster_image'] = array(
			'name'        => __('Poster / fallback image', 'distinctivethemes-panels'),
			'type'        => 'image',
			'group'       => $group,
			'description' => __('This image will be used on devices that don\'t support background video, and will be displayed while the video is loading. We recommend a high-quality screenshot of one of the first few frames. For best results, you should also set this as the background image for this row.', 'distinctivethemes-panels'),
			'priority'    => $order ++,
			'default'     => ''
		);
*/
		// Poster (url)
		$fields['poster_image'] = array(
			'name'        => __('Poster / fallback image URL', 'distinctivethemes-panels'),
			'type'        => 'url',
			'group'       => $group,
			'description' => __('This image will be used on devices that don\'t support background video, and will be displayed while the video is loading. We recommend a high-quality screenshot of one of the first few frames. For best results, you should also set this as the background image for this row.', 'distinctivethemes-panels'),
			'priority'    => $order ++,
		...
		$fields['video_overlay'] = array(
			'name'        => __('Video Overlay', 'distinctivethemes-panels'),
			'type'        => 'select',
			'group'       => $group,
			'description' => __('An overlay can help to provide contrast with your row content, or disguise low quality video.', 'distinctivethemes-panels'),
			'priority'    => $order ++,
			'options'     => array(
								'none'          => 'No overlay',
								'solid'         => 'Color',
		...
		$fields['overlay_opacity'] = array(
			'name'        => __('Overlay Opacity', 'distinctivethemes-panels'),
			'type'        => 'text',
			'group'       => $group,
			'description' => __('Enter a number between 0 and 99. (Only relevant if you choose an overlay type)', 'distinctivethemes-panels'),
			'priority'    => $order ++,
			'default'     => ''
		);
		...
		$fields['overlay_color'] = array(
			'name'        => __('Overlay Color', 'distinctivethemes-panels'),
			'type'        => 'color',
			'group'       => $group,
			'description' => __('(Only relevant if you choose a colored overlay)', 'distinctivethemes-panels'),
			'priority'    => $order ++,
			'default'     => ''
		);
		...
		$fields['fade_in'] = array(
			'name'        => __('Fade in on start?', 'distinctivethemes-panels'),
			'type'        => 'checkbox',
			'default'     => 'true',
			'group'       => $group,
			'description' => __('Fading the video in on start can help avoid distracting the user.', 'distinctivethemes-panels'),
			'priority'    => $order ++
		);

		// Pause after
		$fields['pause_after'] = array(
			'name'        => __('Pause video after...', 'distinctivethemes-panels'),
			'type'        => 'text',
			'default'     => '120',
			'group'       => $group,
			'description' => __('Enter a number of seconds to play before pausing, or 0 for no pause. We recommend pausing after a while and fading out, to reduce your users\' power consumption.', 'distinctivethemes-panels'),
			'priority'    => $order ++
		);

		// Pause button
		$fields['pause_play_button'] = array(
			'name'        => __('Add pause/play button?', 'distinctivethemes-panels'),
			'type'        => 'checkbox',
			'default'     => 'true',
			'group'       => $group,
			'description' => __('It\'s a good idea to allow your user to pause the video if they wish.', 'distinctivethemes-panels'),
			'priority'    => $order ++
		);

		// Pause X Position
		$fields['pauseplay_xpos'] = array(
			'name'        => __('Pause/play button X position', 'distinctivethemes-panels'),
			'type'        => 'select',
			'group'       => $group,
			'description' => __('(Only relevant if you check "Add pause/play button")', 'distinctivethemes-panels'),
			'priority'    => $order ++,
			'options'     => array(
								'left' => 'Left',
								'center' => 'Center',
		...
		$fields['pauseplay_ypos'] = array(
			'name'        => __('Pause/play button Y position', 'distinctivethemes-panels'),
			'type'        => 'select',
			'group'       => $group,
			'description' => __('(Only relevant if you check "Add pause/play button")', 'distinctivethemes-panels'),
			'priority'    => $order ++,
	...
	public function bgvideo_prebuilt_layouts( $layouts ){
		...
		$layouts['example-background-video'] = array (
			'name' => __('Example Background Video', 'distinctivethemes-panels'),
        	'description' => __('A pre-built background video layer with a simple text widget.', 'distinctivethemes-panels'),
		  	'widgets' => 
			  array (
			    0 => 
			    array (
			      'title' => 'Easy HTML5 Background Videos',
			      'text' => 'The Video Backgrounds for distinctivethemes Page Builder plugin lets you quickly and easily add a background video to any page builder row.
...
$bg_vids_for_distinctivethemes = new Background_Videos_For_distinctivethemes();

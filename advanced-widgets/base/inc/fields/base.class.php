...
/**
 * The base class for all DistinctiveThemes_SiteOrigin_Widget form fields.
 *
 * Class DistinctiveThemes_SiteOrigin_Widget_Field
 */

abstract class DistinctiveThemes_SiteOrigin_Widget_Field_Base {
	 ...
	 * the pattern `'distinctivethemes_widgets_sanitize_field_' . $sanitize`, in case the sanitization is defined elsewhere.
	 *
	 * @access protected
	 * @var string
	 */
	 ...
	 *
	 * @access private
	 * @var DistinctiveThemes_SiteOrigin_Widget
	 */
	...
	/* See https://siteorigin.com/docs/advanced-widgets/form-building/state-emitters/ for more detail on the topic of   */
	 ...
	 *
	 * @param DistinctiveThemes_SiteOrigin_Widget $for_widget
	...
	public function __construct( $base_name, $element_id, $element_name, $field_options, $for_widget = null, $parent_container = array() ) {
		if( isset( $field_options['type'] ) ) {
			...
			throw new InvalidArgumentException( 'DistinctiveThemes_SiteOrigin_Widget_Field_Base::__construct: $field_options must contain a \'type\' field.' );
		}
	...
	/**
	 * This function is called by the containing DistinctiveThemes_SiteOrigin_Widget when rendering it's form.
	 *
	...
	protected function render_field_label( $value, $instance ) {
		...
		if( !empty( $this->optional ) ) {
			echo '<span class="field-optional">(' . __('Optional', 'dapper-pro') . ')</span>';
		}
		?>
		</label>
		<?php
		...
		if( isset( $this->sanitize ) ) {
			...
			switch( $this->sanitize ) {
				...
				default:
					...
					$value = apply_filters( 'distinctivethemes_widgets_sanitize_field_' . $this->sanitize, $value );
					break;
			}
		}

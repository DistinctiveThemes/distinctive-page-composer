...
class DistinctiveThemes_SiteOrigin_Widget_Field_TinyMCE extends DistinctiveThemes_SiteOrigin_Widget_Field_Text_Input_Base {
	/**
	...
	function __call( $name, $arguments ) {
		if ( preg_match( '/mce_buttons(?:_[1-4])?|quicktags_settings/', $name ) && !empty( $this->button_filters[$name] ) ) {
			...
			if ( !empty( $filter[0] ) && is_a( $filter[0], 'DistinctiveThemes_SiteOrigin_Widget' ) ) {
				$widget = $filter[0];
	...
	public function enqueue_scripts() {
		wp_enqueue_style( 'dapper-so-tinymce-field', plugin_dir_url(__FILE__) . 'base/inc/fields/css/tinymce-field.css', array(), SOW_BUNDLE_VERSION );
	}

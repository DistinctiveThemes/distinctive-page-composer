...
/**
 * Class DistinctiveThemes_SiteOrigin_Widget_Field_Measurement
 */
class DistinctiveThemes_SiteOrigin_Widget_Field_Measurement extends DistinctiveThemes_SiteOrigin_Widget_Field_Text_Input_Base {
	...
	protected function render_after_field( $value, $instance ) {
		...
		?>
		<select class="dapper-sow-measurement-select-unit siteorigin-widget-input"
				name="<?php echo esc_attr( $this->for_widget->so_get_field_name( $this->base_name . '_unit', $this->parent_container ) ) ?>">
			<?php foreach ( distinctivethemes_widgets_get_measurements_list() as $measurement ):?>
	...
	public function enqueue_scripts() {		
		wp_enqueue_style( 'dapper-so-measurement-field', plugin_dir_url(__FILE__) . 'css/measurement-field.css', array(), SOW_BUNDLE_VERSION );
	}
	...
	protected function sanitize_field_input( $value, $instance ) {
		...
		if( isset( $instance[ $unit_name ] ) ) {
			$units = distinctivethemes_widgets_get_measurements_list();

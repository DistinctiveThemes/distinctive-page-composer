...
/**
 * Class DistinctiveThemes_SiteOrigin_Widget_Field_Repeater
 */
class DistinctiveThemes_SiteOrigin_Widget_Field_Repeater extends DistinctiveThemes_SiteOrigin_Widget_Field_Container_Base {
	...
	protected function render_field( $value, $instance ) {
		...
		if ( ! empty( $item_label ) ) {
			...
			$item_label = distinctivethemes_widgets_underscores_to_camel_case( $item_label );
			$item_label = json_encode( $item_label );
		}
		if( empty( $this->item_name ) ) $this->item_name = __( 'Item', 'dapper-pro' );
		?>
		<div class="siteorigin-widget-field-repeater"
			...
			<?php if( empty( $this->readonly ) ) : ?>
				<div class="siteorigin-widget-field-repeater-add"><?php esc_html_e( 'Add', 'dapper-pro' ) ?></div>
			<?php endif; ?>
			<?php
			ob_start();

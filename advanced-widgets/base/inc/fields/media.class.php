 ...
 *
 * Class DistinctiveThemes_SiteOrigin_Widget_Field_Media
 */
class DistinctiveThemes_SiteOrigin_Widget_Field_Media extends DistinctiveThemes_SiteOrigin_Widget_Field_Base {
	/**
	...
	protected function get_default_options() {
		return array(
			'choose' => __( 'Choose Media', 'dapper-pro' ),
			'update' => __( 'Set Media', 'dapper-pro' ),
			'library' => 'image'
		);
	}
	...
	protected function render_field( $value, $instance ) {
		if( version_compare( get_bloginfo('version'), '3.5', '<' ) ){
			printf( __('You need to <a href="%s">upgrade</a> to WordPress 3.5 to use media fields', 'dapper-pro'), admin_url('update-core.php') );
			return;
		}
		...
		<a href="#" class="media-remove-button <?php if( empty( $value ) ) echo 'remove-hide'; ?>"><?php esc_html_e( 'Remove', 'dapper-pro' ) ?></a>
	...
	protected function render_after_field( $value, $instance ) {
		if( !empty( $this->fallback ) ) {
			...
			<input type="text" value="<?php echo esc_url( $fallback_url ) ?>"
			       placeholder="<?php esc_attr_e( 'External URL', 'dapper-pro' ) ?>"

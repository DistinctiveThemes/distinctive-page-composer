 ...
 *
 * Class DistinctiveThemes_SiteOrigin_Widgets_Widget_Manager
 */
class DistinctiveThemes_SiteOrigin_Widgets_Widget_Manager {
	/**
	 * Regsitered widgets
	 ...
	 *
	 * @return DistinctiveThemes_SiteOrigin_Widgets_Widget_Manager
	 */
	...
	public function register( $id, $path, $class = false ){
		$path = realpath( $path );
		if ( empty( $class ) ) {
			$class = 'DistinctiveThemes_SiteOrigin_Widget_' . str_replace( ' ', '', ucwords( str_replace('-', ' ', $id) ) ) . '_Widget';
		}
	...
	public function get_plugin_path( $id ) {
		if( empty($this->regisrered[$id]) ) {
			...
			if( substr($id, 0, 4) == 'dapper-sow-' ) $id = substr($id, 4);
			else $id = 'dapper-sow-' . $id;
		}
...
DistinctiveThemes_SiteOrigin_Widgets_Widget_Manager::single();
...
function siteorigin_widget_register( $id, $path, $class = false ){
	DistinctiveThemes_SiteOrigin_Widgets_Widget_Manager::single()->register( $id, $path, $class );
}
...
function siteorigin_widget_get_plugin_path($id){
	return DistinctiveThemes_SiteOrigin_Widgets_Widget_Manager::single()->get_plugin_path( $id );
}
...
function siteorigin_widget_get_plugin_dir_path($id){
	return DistinctiveThemes_SiteOrigin_Widgets_Widget_Manager::single()->get_plugin_dir_path($id);
}
...
function siteorigin_widget_get_plugin_dir_url($id){
	return DistinctiveThemes_SiteOrigin_Widgets_Widget_Manager::single()->get_plugin_dir_url($id);
}

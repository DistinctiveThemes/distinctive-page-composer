var soWidgetPostSelector = ( function ($, _) {
    ...
    QueryForm = self.QueryForm = Backbone.View.extend( {
        ...
        render: function(){
            ...
            var orderButton = orderField.closest('.query-builder-form-field').find('.dapper-sow-order-button');

            // Reset the ordering button
            var resetOrderButton = function () {
                if (orderField.val() === 'DESC') {
                    orderButton.removeClass('dapper-sow-order-button-asc');
                    orderButton.addClass('dapper-sow-order-button-desc');
                }
                else {
                    orderButton.addClass('dapper-sow-order-button-asc');
                    orderButton.removeClass('dapper-sow-order-button-desc');
                }
            };
            resetOrderButton();
            ...
            this.$el.find('.query-builder-form-field .dapper-sow-select-posts').click(function(e){
                e.preventDefault();
    ...
    PostCollectionView = self.PostCollectionView = Backbone.View.extend( {
        ...
        render: function(){
            var $c = this.$el.find('.query-builder-content').empty().append('<div class="dapper-sow-current-posts"></div>').find('.dapper-sow-current-posts');
    ...
    PostSelectView = self.PostSelectView = Backbone.View.extend( {
        ...
        render: function(){
            ...
            // Set up the sortable
            this.sortable = this.$el.find('.query-builder-content #dapper-sow-post-selector .dapper-sow-posts-sortable').sortable({
                placeholder: "ui-state-highlight",
                forcePlaceholderSize: true,
                items : '> .dapper-sow-post-selector-summary'
            });
            ...
            var v = this;
            var $searchInput = this.$el.find('.query-builder-content #dapper-sow-post-selector .dapper-sow-search-field');
            ...
            this.$el.find('.query-builder-content').on('click', '.dapper-sow-remove', function(e){
                e.preventDefault();
                var $$ = $(this);
                $$.closest('.dapper-sow-post-selector-summary').fadeOut('fast', function(){
                    $(this).remove();
        ...
        addPosts: function(posts) {
            ...
            for(var i = 0; i < posts.length; i++) {
                if( typeof this.postCache[posts[i]] === 'undefined' ) {
                    ...
                    postItem = $(this.postTemplate( {id:posts[i], title: '', thumbnail: '', editUrl: '#'} )).addClass('dapper-sow-post-loading');
                }
                else {
        ...
        refreshLoading: function(){
            var v = this;
            this.sortable.find('.dapper-sow-post-selector-summary.dapper-sow-post-loading').each(function(){
                var $$ = $(this);
                var id = $$.data('id');

                if(typeof v.postCache[id] !== 'undefined') {
                    $$.removeClass('dapper-sow-post-loading');
        ...
        buttonHandler: function(){
            ...
            var ids = [];
            this.sortable.find('.dapper-sow-post-selector-summary').each(function(){
    ...
    jQuery( function($){
        $('body').on('click', '.dapper-sow-select-posts', function(e){
            e.preventDefault();
            ...
            builder.views.postSummary.posts.on("reset", function (postsCollection) {
                $postSelectorButton.find(".dapper-sow-current-count").text(postsCollection.foundPosts);
            });
            builder.open();
        });
    } );

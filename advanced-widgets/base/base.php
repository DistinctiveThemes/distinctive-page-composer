...
function siteorigin_widget_add_inline_css($css){
	global $distinctivethemes_widgets_inline_styles;
	if(empty($distinctivethemes_widgets_inline_styles)) $distinctivethemes_widgets_inline_styles = '';

	$distinctivethemes_widgets_inline_styles .= $css;
}
...
function siteorigin_widget_print_styles(){
	global $distinctivethemes_widgets_inline_styles;
	if(!empty($distinctivethemes_widgets_inline_styles)) {
		?><style type="text/css"><?php echo($distinctivethemes_widgets_inline_styles) ?></style><?php
	}

	$distinctivethemes_widgets_inline_styles = '';
}
...
function siteorigin_widget_get_icon_list(){
	...
	$widget_icon_families = apply_filters('distinctivethemes_widgets_icon_families', array() );

	header('content-type: application/json');
	echo json_encode( !empty($widget_icon_families[$_GET['family']]) ? $widget_icon_families[$_GET['family']] : array() );
	exit();
}
add_action('wp_ajax_distinctivethemes_widgets_get_icons', 'siteorigin_widget_get_icon_list');
...
function siteorigin_widget_get_icon($icon_value, $icon_styles = false) {
	...
	if( empty($widget_icon_families) ) $widget_icon_families = apply_filters('distinctivethemes_widgets_icon_families', array() );
	...
	if(empty($widget_icons_enqueued[$family]) && !empty($widget_icon_families[$family]['style_uri'])) {
		...
		return '<span class="dapper-sow-icon-' . esc_attr($family) . '" data-dapper-sow-icon="' . $widget_icon_families[$family]['icons'][$icon] . '" ' . ( !empty($icon_styles) ? 'style="'.implode('; ', $icon_styles).'"' : '' ) . '></span>';
	}
	else {
		return false;
	}
...
function siteorigin_widget_get_font($font_value) {
	...
	else if( distinctivethemes_widgets_is_google_webfont( $font_value ) ) {
...
function siteorigin_widget_preview_widget_action(){
	...
	if(!is_a($widget, 'DistinctiveThemes_SiteOrigin_Widget')) exit();

	$instance = json_decode( stripslashes_deep($_POST['data']), true);
	/* @var $widget DistinctiveThemes_SiteOrigin_Widget */
	...
	wp_enqueue_style( 'dapper-so-widget-preview', plugin_dir_url(__FILE__).'/css/preview.css', array(), rand(0,65536) );
...
function siteorigin_widget_search_posts_action(){
	...
	$post_types = apply_filters( 'distinctivethemes_widgets_search_posts_post_types', $post_types );
	...
	echo json_encode( apply_filters( 'distinctivethemes_widgets_search_posts_results', $results ) );
	wp_die();
}
 ...
 * @return mixed|void
 */
function distinctivethemes_widgets_fonts_google_webfonts( ) {
	$fonts = include plugin_dir_path( __FILE__ ) . 'inc/fonts.php';
	$fonts = apply_filters( 'distinctivethemes_widgets_google_webfonts', $fonts );
	return !empty( $fonts ) ? $fonts : array();
}

function distinctivethemes_widgets_is_google_webfont( $font_value ) {
	$google_webfonts = distinctivethemes_widgets_fonts_google_webfonts();
...
function distinctivethemes_widgets_font_families( ){
	...
	foreach ( distinctivethemes_widgets_fonts_google_webfonts() as $font => $variants ) {
	...
	return apply_filters('distinctivethemes_widgets_font_families', $font_families);
}

function distinctivethemes_widgets_tinymce_admin_print_styles() {
	wp_enqueue_style( 'editor-buttons' );
}
add_action( 'admin_print_styles', 'distinctivethemes_widgets_tinymce_admin_print_styles' );
 ...
 * @return array
 */
function distinctivethemes_widgets_get_measurements_list() {
	$measurements = array(
		'px',
		'%',
		'in',
	...
	return apply_filters('distinctivethemes_widgets_get_measurements_list', $measurements);
}

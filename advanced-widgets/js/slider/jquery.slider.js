...
jQuery( function($){
    ...
    var setupActiveSlide = siteoriginSlider.setupActiveSlide = function(slider, newActive, speed){
        ...
        var
            ...
            active = $(newActive),
            video = active.find('video.dapper-sow-background-element');
    ...
    $('.dapper-sow-slider-images').each(function(){
        var $$ = $(this);
        var $p = $$.siblings('.dapper-sow-slider-pagination');
        var $base = $$.closest('.dapper-sow-slider-base');
        var $n = $base.find('.dapper-sow-slide-nav');
        var $slides = $$.find('.dapper-sow-slider-image');
        ...
        var setupSlider = function(){
            ...
            $$.find('.dapper-sow-slider-image').each( function(){
                var $i = $(this);

                $(window)
                    .on('resize panelsStretchRows', function(){
                        $i.css( 'height', $i.find('.dapper-sow-slider-image-wrapper').outerHeight() );
                    })
                    .resize();
            } );
            ...
            $$
                .on({
                    ...
                    'cycle-before' : function(event, optionHash, outgoingSlideEl, incomingSlideEl, forwardFlag) {
                        var $$ = $(this);
                        $p.find('> li').removeClass('dapper-sow-active').eq(optionHash.slideNum-1).addClass('dapper-sow-active');
                    ...
                    'cycle-initialized' : function(event, optionHash){
                        ...
                        $p.find('>li').removeClass('dapper-sow-active').eq(0).addClass('dapper-sow-active');
                ...
                .cycle( {
                    'slides' : '> .dapper-sow-slider-image',
            ...
            $$ .find('video.dapper-sow-background-element').on('loadeddata', function(){
                setupActiveSlide( $$, $$.find( '.cycle-slide-active' ) );
            } );

            // Set up showing and hiding navs
            $p.add($n).hide();
            if( !$base.hasClass('dapper-sow-slider-is-mobile') && $slides.length > 1 ) {

...
global $dapper_cl_eform_index;
$field_name_pattern = isset( $field_name_pattern ) ? $field_name_pattern : '%s';
$field_id_pattern = isset( $field_id_pattern ) ? $field_id_pattern : ( 'dapper_cl_eform_' . $dapper_cl_eform_index . '_%s' );
...
foreach ( $params as $param_name => &$param ) {
	...
	$group = isset( $param['group'] ) ? $param['group'] : __( 'General', 'dapper-pro' );
...
foreach ( $groups as &$group_params ) {
	...
	foreach ( $group_params as $param_name => &$param ) {
		...
		$field_is_shown = isset( $param['show_if'] ) ? dapper_cl_execute_show_if( $param['show_if'], $values ) : TRUE;
		...
		$output .= dapper_cl_get_template( 'eform/' . $param['type'], $field );
		...
		if ( isset( $param['show_if'] ) AND ! empty( $param['show_if'] ) ) {
			$output .= '<div class="cl-eform-row-showif"' . dapper_cl_pass_data_to_js( $param['show_if'] ) . '></div>';
		}

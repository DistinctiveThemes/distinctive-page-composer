...
if ( $link_type != 'none' AND ! empty( $link ) ) {
	$link_atts = dapper_cl_parse_link_value( $link, TRUE );
...
$inline_css = dapper_cl_prepare_inline_css( array(
	'width' => $width,
) );
$output = '<' . $tag . $atts . ' class="cl-flipbox' . $classes . '"' . $inline_css . '>';

$helper_classes = ' easing_' . $easing;
$helper_inline_css = dapper_cl_prepare_inline_css( array(
...
$output .= '<div class="cl-flipbox-front"' . dapper_cl_prepare_inline_css( $front_inline_css ) . '><div class="cl-flipbox-front-h">';
$output_front_icon = '';
if ( $front_icon_type == 'font' AND ! empty( $front_icon_name ) ) {
	...
	$output_front_icon .= '<div class="cl-flipbox-front-icon style_' . $front_icon_style . '"' . dapper_cl_prepare_inline_css( $front_icon_css_props ) . '>';
	$output_front_icon .= '<i class="' . dapper_cl_prepare_icon_class( $front_icon_name ) . '"></i>';
	...
	$output_front_icon .= dapper_cl_prepare_inline_css( array(
...
if ( ! empty( $front_title ) ) {
	...
	$output_front_title .= dapper_cl_prepare_inline_css( array(
...
if ( ! empty( $front_desc ) ) {
	...
	$output_front_desc .= dapper_cl_prepare_inline_css( array(
		'color' => $front_textcolor,
	) );
...
$output .= '<div class="cl-flipbox-back"' . dapper_cl_prepare_inline_css( $back_inline_css ) . '><div class="cl-flipbox-back-h">';
$output_back_title = '';
if ( ! empty( $back_title ) ) {
	$output_back_title .= '<h4 class="cl-flipbox-back-title"';
	$output_back_title .= dapper_cl_prepare_inline_css( array(
...
if ( ! empty( $back_desc ) ) {
	...
	$output_back_desc .= dapper_cl_prepare_inline_css( array(
		'color' => $back_textcolor,
	) );
...
if ( $link_type == 'btn' AND isset( $link_atts ) AND ! empty( $back_btn_label ) ) {
	$back_btn_inline_css = dapper_cl_prepare_inline_css( array(
...
if ( $animation == 'cubeflip' ) {
	...
	if ( in_array( $direction, array( 'ne', 'e', 'se', 'sw', 'w', 'nw' ) ) ) {
		...
		$front_rgb = dapper_cl_hex_to_rgb( $front_bgcolor );
		for ( $i = 0; $i < 3; $i++ ) {
			$front_rgb[ $i ] = min( 250, $front_rgb[ $i ] + 20 );
		}
		$flank_inline_css_props['background-color'] = dapper_cl_rgb_to_hex( $front_rgb );
		$output .= '<div class="cl-flipbox-yflank"' . dapper_cl_prepare_inline_css( $flank_inline_css_props ) . '></div>';
	}
	if ( in_array( $direction, array( 'n', 'ne', 'se', 's', 'sw', 'nw' ) ) ) {
		...
		$flank_inline_css_props['background-color'] = dapper_cl_rgb_to_hex( $front_rgb );
		$output .= '<div class="cl-flipbox-xflank"' . dapper_cl_prepare_inline_css( $flank_inline_css_props ) . '></div>';
	}
}

...
$atts = empty( $link ) ? '' : dapper_cl_parse_link_value( $link, TRUE );
...
if ( ! empty( $title ) ) {
	...
	$title_html .= dapper_cl_prepare_inline_css( array(
...
if ( ! empty( $desc ) ) {
	...
	$desc_html .= dapper_cl_prepare_inline_css( array(
		'font-size' => $desc_size,
	) );
	$desc_html .= '>' . $desc . '</div>';
	$classes .= ' with_desc';
...
if ( ! empty( $mobile_css_rules ) ) {
	...
	foreach ( $mobile_css_rules as $mobile_css_elm => $mobile_css_rule ) {
		$custom_css .= '.dapper_cl_custom_css_' . $dapper_cl_custom_css_id . ' ' . $mobile_css_elm . ' { ' . $mobile_css_rule . ' }';
	}
	$custom_css .= '}';
	$custom_css .= '</style>';
}
...
$output .= dapper_cl_prepare_inline_css( array(
	'width' => $width,

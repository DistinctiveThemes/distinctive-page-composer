...
if ( ! empty( $font_size_mobile ) AND (int) $font_size_mobile != (int) $font_size ) {
	...
	global $dapper_cl_custom_css_id;
	$dapper_cl_custom_css_id = isset( $dapper_cl_custom_css_id ) ? ( $dapper_cl_custom_css_id + 1 ) : 1;
	$custom_css .= '<style type="text/css">';
	$custom_css .= '@media only screen and (max-width: 599px) {';
	$custom_css .= '.dapper_cl_custom_css_' . $dapper_cl_custom_css_id . ' { font-size: ' . intval( $font_size_mobile ) . 'px !important; }';
	$custom_css .= '}';
	$custom_css .= '</style>';
	$classes .= ' dapper_cl_custom_css_' . $dapper_cl_custom_css_id;
}

$output = '<' . $tag . ' class="cl-itext' . $classes . '"' . $inline_css . dapper_cl_pass_data_to_js( $js_data ) . '>';
foreach ( $groups as $index => $group ) {
	ksort( $group );
	if ( empty( $group_changes[ $index ] ) ) {
		...
		if ( in_array( '0', $group_changes[ $index ] ) ) {
			...
			$output .= ' dynamic"' . dapper_cl_prepare_inline_css( array( 'color' => $dynamic_color ) );
		} else {
			$output .= '"';
		}
		$output .= dapper_cl_pass_data_to_js( $group ) . '>' . $group[0] . '</span>';
	}
}
$output .= '</' . $tag . '>';

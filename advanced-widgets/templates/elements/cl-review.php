...
$author_html .= dapper_cl_prepare_inline_css( array(
	'font-size' => $author_size,
) );
$author_html .= '>';
if ( ! empty( $source ) ) {
	$author_html .= '<a' . dapper_cl_parse_link_value( $source, TRUE ) . '>';
}
...
if ( ! empty( $quote ) ) {
	...
	$output .= '<div class="cl-review-quote-text"' . dapper_cl_prepare_inline_css( $quote_inline_css ) . '>';
	if ( $layout == 'modern' ) {

 ...
 * @return bool
 */
function dapper_cl_maybe_load_plugin_textdomain( $domain = 'dapper-pro', $path = 'lang' ) {
	if ( is_textdomain_loaded( $domain ) ) {
		return TRUE;
	}
	global $dapper_cl_dir;

	return load_plugin_textdomain( $domain, FALSE, basename( $dapper_cl_dir ) . '/' . $path );
}
 ...
 * @return mixed
 */
function dapper_cl_config( $path, $default = NULL ) {
	global $dapper_cl_dir;
	...
	if ( ! isset( $configs[ $config_name ] ) ) {
		...
		$config_path = $dapper_cl_dir . 'config/' . $config_name . '.php';
		$config = require $config_path;
		$configs[ $config_name ] = apply_filters( 'dapper_cl_config_' . $config_name, $config );
	}
 ...
 */
function dapper_cl_load_template( $template, $vars = NULL ) {
	global $dapper_cl_dir;

	$vars = apply_filters( 'dapper_cl_template_vars:' . $template, $vars );
	if ( is_array( $vars ) AND ! empty( $vars ) ) {
		extract( $vars );
	}

	do_action( 'dapper_cl_before_template:' . $template, $vars );
	if ( ! file_exists( $dapper_cl_dir . 'templates/' . $template . '.php' ) ) {
		wp_die( 'File not found: ' . $dapper_cl_dir . 'templates/' . $template . '.php' );
	}
	include $dapper_cl_dir . 'templates/' . $template . '.php';
	do_action( 'dapper_cl_after_template:' . $template, $vars );
}
 ...
 */
function dapper_cl_get_template( $template, $vars = NULL ) {
	ob_start();
	dapper_cl_load_template( $template, $vars );
 ...
 * @return array Result
 */
function dapper_cl_shortcode_atts( $atts, $shortcode ) {

	// We need to extract shortcodes pairs from the config, so storing them within app execution for productivity
	global $dapper_cl_shortcode_pairs;

	if ( ! isset( $dapper_cl_shortcode_pairs ) ) {
		$dapper_cl_shortcode_pairs = array();
	}

	if ( ! isset( $dapper_cl_shortcode_pairs[ $shortcode ] ) ) {
		$dapper_cl_shortcode_pairs[ $shortcode ] = array();
		foreach ( dapper_cl_config( 'elements.' . $shortcode . '.params', array() ) as $param_name => $param ) {
			if ( ! isset( $param_name ) ) {
				continue;
			}
			if ( isset( $param['std'] ) ) {
				$dapper_cl_shortcode_pairs[ $shortcode ][ $param_name ] = $param['std'];
			} elseif ( $param['type'] == 'select' AND isset( $param['options'] ) AND is_array( $param['options'] ) ) {
				$dapper_cl_shortcode_pairs[ $shortcode ][ $param_name ] = key( $param['options'] );
			} else {
				$dapper_cl_shortcode_pairs[ $shortcode ][ $param_name ] = '';
			}
		}
	}

	return shortcode_atts( $dapper_cl_shortcode_pairs[ $shortcode ], $atts, $shortcode );
}
 ...
 * @return mixed
 */
function dapper_cl_parse_link_value( $value, $as_string = FALSE ) {
 ...
 * The configration will be available in JavaScript: tinyMCEPreInit.mceInit['dapper-pro']
 */
function dapper_cl_maybe_load_wysiwyg() {
	global $dapper_cl_html_editor_loaded;
	if ( ! isset( $dapper_cl_html_editor_loaded ) OR ! $dapper_cl_html_editor_loaded ) {
		$screen = get_current_screen();
		if ( $screen->base == 'customize' ) {
			dapper_cl_load_wysiwyg();
		} else {
			// Support for 3-rd party plugins that customize mce_buttons during the admin_head action
			add_action( 'admin_head', 'dapper_cl_load_wysiwyg', 50 );
		}
		$dapper_cl_html_editor_loaded = TRUE;
	}
}

function dapper_cl_load_wysiwyg() {
	if ( ! class_exists( '_WP_Editors' ) ) {
		require( ABSPATH . WPINC . '/class-wp-editor.php' );
	}
	_WP_Editors::editor_settings( 'dapper-pro', _WP_Editors::parse_settings( 'content', array(
		'dfw' => TRUE,
 ...
 * @return string
 */
function dapper_cl_prepare_icon_class( $icon_class ) {
 ...
 * @return string
 */
function dapper_cl_prepare_inline_css( $props, $style_attr = TRUE ) {
	$result = '';
 ...
 * @return array
 */
function dapper_cl_get_intermediate_image_size( $size_name ) {
 ...
 * @return array
 */
function dapper_cl_image_sizes_select_values( $size_names = array( 'large', 'medium', 'thumbnail', 'full' ) ) {
	$image_sizes = array();
	// For translation purposes
	$size_titles = array(
		'large' => __( 'Large', 'dapper-pro' ),
		'medium' => __( 'Medium', 'dapper-pro' ),
		'thumbnail' => __( 'Thumbnail', 'dapper-pro' ),
		'full' => __( 'Full Size', 'dapper-pro' ),
	);
	foreach ( $size_names as $size_name ) {
		$size_title = isset( $size_titles[ $size_name ] ) ? $size_titles[ $size_name ] : ucwords( $size_name );
		if ( $size_name != 'full' ) {
			// Detecting size
			$size = dapper_cl_get_intermediate_image_size( $size_name );
			$size_title .= ' - ' . ( ( $size['width'] == 0 ) ? __( 'Any', 'dapper-pro' ) : $size['width'] );
			$size_title .= 'x';
			$size_title .= ( $size['height'] == 0 ) ? __( 'Any', 'dapper-pro' ) : $size['height'];
			$size_title .= ' (' . ( $size['crop'] ? __( 'cropped', 'dapper-pro' ) : __( 'not cropped', 'dapper-pro' ) ) . ')';
		}
 ...
 */
function dapper_cl_pass_data_to_js( $data ) {
 ...
 * @return array
 */
function dapper_cl_hex_to_rgb( $hex ) {
 ...
 * @return string
 */
function dapper_cl_rgb_to_hex( $rgb ) {
 ...
 */
function dapper_cl_array_to_data_js( $data ) {

...
class CL_Widget extends WP_Widget {
	...
	public function __construct( $id_base ) {
		...
		$this->config = dapper_cl_config( 'elements.' . $id_base );
		...
		$this->config['params'] = array_merge( array(
			'_widget_title' => array(
				'title' => __( 'Widget Title', 'dapper-pro' ),
				'type' => 'textfield',
			),
		), $this->config['params'] );
	...
	public function form( $instance ) {
		...
		dapper_cl_load_template( 'eform/eform', array(
			'name' => $this->id_base,
	...
	public function widget( $args, $instance ) {
		...
		$instance = dapper_cl_shortcode_atts( $instance, $this->id_base );
		dapper_cl_load_template( 'elements/' . $this->id_base, $instance );
...
add_action( 'widgets_init', 'dapper_cl_widgets_init' );
function dapper_cl_widgets_init() {
